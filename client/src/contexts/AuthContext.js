...
const AuthContextProvider = ({children}) => {
    ...
    async function loadUser ()  {
        if(localStorage[LOCAL_STORED_TOKEN_NAME]) {
            setAuthToken(localStorage[LOCAL_STORED_TOKEN_NAME])
        }
        try {
            const res = await axios.get(`${apiUrl}/auth`)
            if(res.data.success) {
                console.log("SET_AUTH REDUCER")
                dispatch({
                    type: "SET_AUTH",
            ...
            localStorage.removeItem(LOCAL_STORED_TOKEN_NAME)
            setAuthToken(null)
            dispatch({
                type: "SET_AUTH",
